    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                elif args.defense == "mostleast":
                    preds = F.softmax(outputs[1], dim=-1)
                    max_idx = preds.argmax(dim=-1)
                    min_idx = preds.argmin(dim=-1)
                    row_ids = torch.arange(len(preds)).to(device=preds.device)
                    peturb_probs = torch.zeros_like(preds)
                    peturb_probs[row_ids, max_idx] = 0.5 + 1e-5
                    peturb_probs[row_ids, min_idx] = 0.5 - 1e-5
                    preds = peturb_probs.detach().cpu().numpy()
                else:
...
def main():
    ...
    parser.add_argument("--defense", default="temp", type=str, choices=['temp', 'perturb', 'mostleast'], help="Defense strategy")
